...
//                                           print_r($service_amount1);exit;
                                            ...
                                            if (is_nan($service_amount1) || is_infinite($service_amount1)) {
                                             ...
                                             //echo 'yrrr<pre>'; print_r($all_bookings);
                                            ?>
                                                        ...
                                                        if($this->session->userdata('usertype') == "user") { 
                                                            if($userId && ($userId == $blocked_user['blocked_by_id']) && ($provider_info['id'] == $blocked_user['blocked_id']) ) {
                                                                ...
                                                                <?php }
                                                                    ...
                                                                    <!--<a href="javascript:;" class="myBlockReasonModal" data-doggle="modal" data-target="#myBlockReasonModal" style="float: right;" data-id="<?php echo $blocked_user['id']?>" data-blockedbyid = "<?php echo $userId?>" data-blockedid="<?php echo $blocked_user['blocked_id']?>" data-blockedstatus="2" data-usertype="1"><span class='badge badge-pill badge-prof <?php echo $blk_class; ?>'><?= $blk_text; ?></span></a> -->
                                                                <?php } 
                                                            } else { ?>
                                                               <!-- <a style="float: right;" href="javascript:;" class="myBlockReasonModal"  data-doggle="modal" data-target="#myBlockReasonModal"  data-id="" data-blockedbyid = "<?php echo $this->session->userdata('id');?>" data-blockedid="<?php echo $provider_info['id']?>" data-blockedstatus="2" data-usertype="1"><span class='badge badge-pill badge-prof bg-danger'><?= $blk_text; ?></span></a> -->
                                                            <?php }
                                                        }
                                                    ?>
                                                </li>
                                    ...
                                    <div class="booking-action">
                                        ...
                                        <?php $rating_count = array(); if($blocked_user['status'] != 1) { //add new line?>
                                        <?php $pending = 0; 
                                        $rating_count1 = $this->db->where('user_id', $this->session->userdata('id'))->where('booking_id', $bookings['id'])->get('rating_review');
                                        $query = $this->db->get();
										
                                        if($query !== FALSE && $query->num_rows() > 0){
                                            $rating_count = $query->row();
                                        }
                                        ?>
