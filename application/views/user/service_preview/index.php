            ...
            <div class="col-lg-8">
                ...
                <div class="service-view">
                    <div class="service-header">
                        <h1><?php echo ucfirst($service['service_title']); ?>
                        ...
                        if($this->session->userdata('usertype') != "provider") {    
                            ...
                            if($userId && ($userId == $user_fav['user_id']) ) {
                                if($user_fav['status'] == 1) { ?>
                                    <a href="javascript:;" id="ufav" class="hearting" style="" data-id="<?php echo $user_fav['id']?>" data-userid = "<?php echo $userId?>" data-provid="<?php echo $service['user_id']?>" data-servid="<?php echo $service['id']?>" data-favstatus="0" data-pagename="<?php echo $service['category_name']?>" title="Click here to unfavorite the service"><i class="fas fa-heart filled"></i></a>
                                <?php } 
                                else { ?>
                                    <a href="javascript:;" id="ufav" class="hearting" style="color: #242423;" data-id="<?php echo $user_fav['id']?>" data-userid = "<?php echo $userId?>" data-provid="<?php echo $service['user_id']?>" data-servid="<?php echo $service['id']?>" data-favstatus="1" data-pagename="<?php echo $service['category_name']?>"  title="Click here to favorite the service"><i class="fas fa-heart"></i></a>
                                <?php } 
                            } else { ?>
                                <a href="javascript:;" id="ufav" class="hearting"  style="color: #242423;" data-id="<?php echo $user_fav['id']?>" data-userid = "<?php echo $this->session->userdata('id');?>" data-provid="<?php echo $service['user_id']?>" data-servid="<?php echo $service['id']?>" data-favstatus="1" data-pagename="<?php echo $service['category_name']?>" title="Click here to favorite the service"><i class="fas fa-heart"></i></a>
                            <?php } 
                        }?>
						</h1>
                    ...
                    <div class="service-book">
                        ...
                        if (!empty($userId)) {
                            if (!empty($usertype) && $usertype == 'user') {
                                ...
                                <?php /*$wallet_info = $this->db->select('*')->from('wallet_table')->where($where)->get()->row();
                                if (isset($wallet_info->wallet_amt)) {
                                    ...
                                    <?php } else { ?>
                                        <button class="btn btn-primary" id="add_wallet_money" data-url="<?php echo base_url() . 'service-preview/' . str_replace(' ', '-', $serv['service_title']) . '?sid=' . md5($serv['id']); ?>" type="button" ><?php echo (!empty($user_language[$user_selected]['lg_Book_Service'])) ? $user_language[$user_selected]['lg_Book_Service'] : $default_language['en']['lg_Book_Service']; ?></button>
                                        <?php
                                    }
                                } */
                                ?>
                    ...
                    <div class="card-body">
                        ...
                        if (!empty($service['user_id'])) {
                            ...
                            <?php /*
                                ...
                                $this->db->where('blocked_id', $service['user_id']);
                               ...
                               // $blocked_user = $this->db->get()->row_array();
                                ...
                                if($blocked_user['status'] == "1") {
                                    $blk_class = "btn-danger";
                                    $blk_text = "Blocked";
                                }else if($blocked_user['status'] == "2") {
                                    $blk_class = "btn-info";
                                    $blk_text ="Requested to block provider"; 
                                }
                                else{
                                    $blk_class = "btn-danger";
                                    $blk_text = "Block";
                                ...
                                $userId = $this->session->userdata('id');
                                if($this->session->userdata('usertype') == "user") { 
                                    if($userId && ($userId == $blocked_user['blocked_by_id']) && ($service['user_id'] == $blocked_user['blocked_id']) ) {
                                        if($blocked_user['status'] == 1) { ?>
                                            <!-- <a href="javascript:;" class="blockingUser" style="float: right;" data-id="<?php echo $blocked_user['id']?>" data-blockedbyid = "<?php echo $userId?>" data-blockedid="<?php echo $blocked_user['blocked_id']?>" data-blockedstatus="0" data-usertype="2" ><span class='badge badge-pill badge-prof <?php echo $blk_class; ?>'><?= $blk_text; ?></span></a> -->
                                            <span class='btn <?php echo $blk_class; ?>'><?= $blk_text; ?></span>
                                        <?php } else if($blocked_user['status'] == 2) { ?>
                                            <!-- <a href="javascript:;" class="blockingUser" style="float: right;" data-id="<?php echo $blocked_user['id']?>" data-blockedbyid = "<?php echo $userId?>" data-blockedid="<?php echo $blocked_user['blocked_id']?>" data-blockedstatus="0" data-usertype="2" ><span class='badge badge-pill badge-prof <?php echo $blk_class; ?>'><?= $blk_text; ?></span></a> -->
                                            <span class='btn <?php echo $blk_class; ?>'><?= $blk_text; ?></span>
                                        <?php }
                                        else { ?>
                                            <a href="javascript:;" class="myBlockReasonModal" data-doggle="modal" data-target="#myBlockReasonModal" style="float: right;" data-id="<?php echo $blocked_user['id']?>" data-blockedbyid = "<?php echo $userId?>" data-blockedid="<?php echo $blocked_user['blocked_id']?>" data-blockedstatus="2" data-usertype="1"><span class='btn <?php echo $blk_class; ?>'><?= $blk_text; ?></span></a>
                                        <?php } 
                                    } else { ?>
                                        <a href="javascript:;" class="myBlockReasonModal" data-doggle="modal" data-target="#myBlockReasonModal" tyle="float: right;" data-id="" data-blockedbyid = "<?php echo $this->session->userdata('id');?>" data-blockedid="<?php echo $service['user_id']?>" data-blockedstatus="2" data-usertype="1"><span class='btn btn-danger'><?= "Block" ?></span></a>
                                    <?php }
                                } */
                            ?>
