						...
						<div class="table-responsive total-booking-report">
							<table class="table table-hover table-center mb-0 service_table" >
                                ...
                                <tbody>
									...
									if ($rows['status']==5) {
										if($rows['admin_update_status'] == 0) { $badge='Rejected by User'; }
										else  { $badge='Rejected'; }
										$color='warning';
									} 
									if ($rows['status']==6) {
									...
									if ($rows['status']==7) {
										if($rows['admin_update_status'] == 0) { $badge='Cancelled by Provider'; }
...
<style type="text/css">
.total-booking-report label.badge{padding: 0.55em 1em !important;}
</style>