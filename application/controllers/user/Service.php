...
class Service extends CI_Controller {
 ...
 $query = $this->db->query("select * from system_settings WHERE status = 1");
        ...
        if ($this->input->post('form_submit')) {
           ...
           // echo 'test<pre>'; print_r($inputs); exit;
    ...
    public function update_bookingstatus() {
        ...
        if($result){
            ...
            if($book_details['status'] ==7){
                $token=$this->session->userdata('chat_token');
                $this->send_push_notification($token,$book_details['id'],2,' Have Cancelled The Service - '.$service_data['service_title']);
			...
			//$body=$this->load->view('user/email/service_request_email',$this->data,true);
			$bodyid = 4;
			...
			$sms_config=settingValue('sms_key');
			$sms_secret_key=settingValue('sms_secret_key');
			$sms_sender_id=settingValue('sms_sender_id');  
            ...
            $nexmo_sms->sendText($user_data['country_code'].$user_data['mobileno'],$sms_sender_id,$smsmessage);
			 ...
			 // $this->sms->send_message($user_data['country_code'].$user_data['mobileno'],$smsmessage);
			}
            echo "1";
		} else {
            $message = 'Something went wrong.Please try again later.';
         ...
         $result=$this->service->update_bookingstatus($book_details,$WHERE);
		  ...
		  if($result){
			...
			if($book_details['status'] ==6){
			 ...
			 $this->send_push_notification($token,$book_details['id'],1,' Have Accepted The Service - '.$service_data['service_title']);
		   ...
		   if($book_details['status'] ==5){
			 ...
			 $token=$this->session->userdata('chat_token');
			 $this->send_push_notification($token,$book_details['id'],1,' Have Rejected The Service - '.$service_data['service_title']);
            ...
            $nexmo_sms->sendText($provider_data['country_code'].$provider_data['mobileno'],$sms_sender_id,$smsmessage);
			  ...
			  //$this->sms->send_message($provider_data['country_code'].$provider_data['mobileno'],$smsmessage);
			}
		   if($book_details['status'] ==7){
