...
class Dashboard extends CI_Controller {
  ...
  public function __construct() 
  {
    ...
    //echo '<pre>'; print_r($this->data['payment'][0]->paid_amt); exit;
    //Currency Convertion 
  ...
  public function users_list($value='')
  {
    ...
    else
    {
      ...
      foreach ($lists as $template) {
        ...
        <a href='javascript:;'' class='on-default remove-row btn btn-sm bg-danger-light mr-2 delete_user_data' id='Onremove_'".$template->id."' data-id='".$template->id."'><i class='far fa-trash-alt mr-1'></i> Delete</a>";        
...
public function adminusers_list($value='')
{
  ...
  public function update_adminuser()
  {
    ...
    if (!empty($profile_image)) { 
      //$params['profile_img'] = "uploads/profile_img/" . $profile_image;
...
public function SendPushNotification()
{
	...
	if($this->input->post())
	{
		...
		//echo '<pre>';print_r($provider_access);exit;
	}
	...
	public function update_user()
  {
    ...
    if(!empty($params['id'])){
      ...
      //insert chat
      $chat_arr = ['sender_token'=>'0dreamsadmin', 'receiver_token'=>$token, 'message'=>'Hi! Welcome to TruelySell', 'status'=>'1', 'read_status'=>'0', 'utc_date_time'=>date('Y-m-d H:i:s')];
