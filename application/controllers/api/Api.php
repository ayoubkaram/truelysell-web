        ...
        if (!empty($token)) {
            ...
            if (!empty($history)) {
                ...
                foreach ($history as $key => $value) {
                    ...
                    $his[$key]['current_wallet'] = number_format(get_gigs_currency($value['current_wallet'], $value['currency_code'], $UserCurrency), 2);
                    $his[$key]['credit_wallet'] = number_format(get_gigs_currency($value['credit_wallet'], $value['currency_code'], $UserCurrency), 2);
                    $his[$key]['debit_wallet'] = number_format(get_gigs_currency(strval(abs($value['debit_wallet'])), $value['currency_code'], $UserCurrency), 2);
                    $his[$key]['avail_wallet'] = number_format(get_gigs_currency($value['avail_wallet'], $value['currency_code'], $UserCurrency), 2);
					if($value['payment_detail'] == 'paypal' || $value['payment_detail'] == 'Razorpay'){
						$his[$key]['total_amt'] = number_format(get_gigs_currency(strval(abs($value['total_amt'])), $value['currency_code'], $UserCurrency), 2);
						$his[$key]['txt_amt'] = number_format(get_gigs_currency(strval(abs($value['fee_amt'])), $value['currency_code'], $UserCurrency), 2);						
					}else{
						$his[$key]['total_amt'] = number_format(get_gigs_currency(strval(abs($value['total_amt'] / 100)), $value['currency_code'], $UserCurrency), 2);
...
//                                    print_r($amount);exit;
    ...
    {
		//print_r($this->post());echo $this->user_id; exit;
    	if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
    		$user_data = $this->post();
            ...
            if (!empty($service)) 
            {
				...
				//echo $this->db->last_query(); print_r($getdata);;				
				//echo count($getdata); //exit;
				if(count($getdata)==0) {
    ...
    public function userfavorites_get()
    {
        if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
			$userfav = $this->api->getUserFavorites($this->users_id);			
			if (!empty($userfav)) 
			{
				$response_code = '200';
				$response_message = "User Favorites Services Listed Out Successfully";
				$data['userfavorites'] = $userfav;
			} else {
				$response_code = '500';
				$response_message = "No Details found";
				$data = new stdClass();
	...
	public function countries_get() {
        ...
        $response_code = 200;
        $response_message = "Fetched Successfully...";
        $result = $data; //$this->data_format($response_code, $response_message, $data);
        $this->response($result, REST_Controller::HTTP_OK);
    }
